...
class Search {
	...
	public static int max(ICanCompare[] objs) {
		int max = 0;
		for(int i = 1;i < objs.length;i++)
			max = (objs[max].compare(objs[i]) > 0) ? max :i;
		return max;
	}

	public static int min(ICanCompare[] objs) {
		int min = 0;
		for(int i = 1;i < objs.length;i++)
			min = (objs[min].compare(objs[i]) < 0) ? min :i;
		return min;
	}
}
...
public class Pgm1206 {
	...
	public static void main(String[] argv) {
		...
		int pos = Search.max(persons);
		System.out.println("姓名排最後的是 " + persons[pos]); 
		pos = Search.min(persons);
		System.out.println("姓名排最前的是 " + persons[pos]);
	}
}